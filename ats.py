...
def create_pdf_report(resume_text, analysis_result, overall_score, ai_analysis):
    ...
    # Title
    story.append(Paragraph("Candidate Profile Report", title_style))
    ...
    story.append(Paragraph(f"Name: {profile.get('name', 'Not found')}", normal_style))
    story.append(Paragraph(f"Email: {profile.get('email', 'Not found')}", normal_style))
    story.append(Paragraph(f"Phone: {profile.get('phone', 'Not found')}", normal_style))
    story.append(Spacer(1, 12))
...
def generate_report(resume_text, analysis_result, overall_score, ai_analysis):
...
* Work History: {profile.get('experience', 'Not found')}
...
# 3. Overall ATS Score: {overall_score}
...
def get_binary_file_downloader_html(bin_file, file_label='File'):
    """Generate a download link for the binary file."""
...
# Function to extract relevant sections from the job description
...
if uploaded_file is not None:
    ...
    if st.button("Analyze"):
        if job_description and resume_text:
            # Get AI analysis
            analysis_result = analyze_with_google_ai(resume_text, job_description)
            
            # Parse AI response
            ai_analysis = parse_ai_response(analysis_result)
            
            # Calculate bonus score
            bonus_score = calculate_bonus_score(ai_analysis)

            # Display AI analysis results
            st.subheader("Analysis Results")
            st.write(analysis_result)

            job_description_sections = extract_jd_sections(job_description)

            # Calculate all scores
            ats_score = ats_rules_check(resume_text, job_description_sections)
            match_percentage = calculate_jd_match(resume_text, job_description)
            skill_match_score = score_skill_matching(resume_text, job_description_sections)
            experience_match_score = score_experience_matching(resume_text, job_description_sections)
            education_match_score = score_education_matching(resume_text, job_description_sections)
            keyword_density_score = score_keyword_density(resume_text, job_description)
            resume_quality_score = score_resume_quality(resume_text)

            # Calculate overall score with bonus
            overall_score = calculate_overall_score(
                ats_score, match_percentage, skill_match_score,
                experience_match_score, education_match_score,
                keyword_density_score, resume_quality_score, bonus_score
            )

            # Display results
            st.subheader("Overall Score: {}".format(overall_score))

            # Display red flags
            red_flags = identify_red_flags(resume_text, job_description_sections)
            for flag in red_flags:
                st.write(flag)
        else:
            st.warning("Please ensure both the resume and job description are provided.")
        if 'analysis_result' in locals() and 'overall_score' in locals():
            st.subheader("Candidate Profile Report")
            
            # Generate the PDF report
            pdf_buffer = create_pdf_report(resume_text, analysis_result, overall_score, ai_analysis)
            
            # Display download button for PDF
            st.markdown(
                get_binary_file_downloader_html(pdf_buffer, 'candidate_report.pdf'),
                unsafe_allow_html=True
            )
            
            # Create a button to copy report to clipboard
            if st.button("Copy to Clipboard"):
                st.write("Report copied to clipboard!")
                st.session_state['clipboard'] = analysis_result

# Function to extract more detailed profile fields
